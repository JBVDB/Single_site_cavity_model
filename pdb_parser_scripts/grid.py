...
# Enum for different types of coordinate system
...
def define_coordinate_system(pos_N, pos_CA, pos_C, z_direction):
    ...
    # Define local coordinate system (local eigenbasis)
    e1 = pos_C - pos_N
    ...
    e3 = np.cross(e1, e2) # computes the cross-product

    # N-C and e2 are not perfectly perpendical to one another. We adjust e2.
    e2 = np.cross(e1, -e3) # since e3 IS perpendicular to e1 and e2, we can recompute e2 (distdaorsion)
